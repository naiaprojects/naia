...
export default function ShareButtons({ slug, title }) {
    ...
    useEffect(() => {
        // Ambil full URL dari browser
        if (typeof window !== 'undefined') {
            const baseUrl = window.location.origin;
            setFullUrl(`${baseUrl}/pages/${slug}`);
        }
    }, [slug]);

    // Encode URL dan title untuk sharing
    ...
    // Jangan render sampai URL ready
    ...
    return (
        <div className="flex flex-wrap items-center gap-2">
            ...
            <button
                ...
                className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-slate-600 hover:bg-slate-700 text-white transition-colors relative"
                title="Copy Link"
            >
                {copied ? (
            ...
            {/* Toast notification untuk copied */}
            {copied && (
                <span className="ml-2 text-sm text-green-600 font-medium animate-pulse">
                    Link disalin!
                </span>
            )}
        </div>
