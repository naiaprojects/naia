...
const OrderConfirmation = () => {
  ...
  const handleWhatsAppClick = async () => {
    if (paymentData && paymentData.customer) {
      ...
      const phoneNumber = '6281320858595'; // Nomor WhatsApp admin
      
      // Buka WhatsApp
      window.open(`https://wa.me/${phoneNumber}?text=${encodeURIComponent(message)}`, '_blank');
      
      // Kirim notifikasi ke admin via API watsap.id
      try {
        const response = await fetch('/api/notify-admin', {
          ...
          body: JSON.stringify({
            message: `Pelanggan ${paymentData.customer.name} (${paymentData.customer.phone}) telah menghubungi via WhatsApp untuk konfirmasi pesanan ${paymentData.invoiceNumber}`,
        ...
        if (response.ok) {
          ...
          console.error('Gagal mengirim notifikasi admin');
        }
      } catch (error) {
